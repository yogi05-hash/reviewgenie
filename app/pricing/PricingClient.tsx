...
export default function PricingClient() {
  ...
  const handleUpgrade = async () => {
    ...
    if (error) { alert('Something went wrong. Try again.'); setLoading(false); return }
      ...
      <section style={{ maxWidth: '900px', margin: '0 auto', padding: '80px 40px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px', alignItems: 'start' }}>
          ...
          <div style={{ backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', padding: '36px 28px', textAlign: 'left' }}>
            ...
            <Link href="/signup" style={{ display: 'block', border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.7)', padding: '12px', borderRadius: '10px', textDecoration: 'none', fontSize: '14px', fontWeight: 600, textAlign: 'center' }}>
              Get started free
            </Link>
          </div>
          ...
          <div style={{ backgroundColor: '#fff', borderRadius: '20px', padding: '36px 28px', textAlign: 'left', position: 'relative', color: '#0a0a0f' }}>
            ...
            <button onClick={handleUpgrade} disabled={loading}
              style={{ display: 'block', width: '100%', backgroundColor: '#0a0a0f', color: '#fff', padding: '13px', borderRadius: '10px', border: 'none', fontSize: '15px', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}>
              {loading ? 'Loading checkout...' : 'Start Pro — £49/mo'}
            </button>
          </div>
        ...
        {/* FAQ */}
        <div style={{ marginTop: '80px', maxWidth: '600px', margin: '80px auto 0', textAlign: 'left' }}>
